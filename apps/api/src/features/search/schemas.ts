...
export const SearchRequestSchema = z
  .object({
    ...
    recall_id: UuidSchema.optional().openapi({
      description:
        'Optional stable id for one logical recall. Retries of the same logical ' +
        'search should reuse the same value: the server then reuses a single ' +
        'concurrency slot instead of counting each retry as a new concurrent ' +
        'search. Omit it and behavior is unchanged. Generate a fresh id per ' +
        'distinct search — reusing one id across genuinely different searches ' +
        'makes them share a slot.',
    }),
  })
