...
production incident. The empty staging branch was backed up and reconciled
through committed migrations `0001`–`0004` plus the archived scoped-API-key
change. Its 451 semantic column/index/FK definitions now match the
migration-built reference apart from the equivalent legacy sequence name; all
indexes are valid. The next scheduled redrive completed on its first attempt at
`10:30:17Z` with zero candidates/jobs and no post-repair sweep failure, closing
the schema gate. The operator opened the controlled security-endpoint trace
...
from its usage page. The destination jobs completed error-free after rollout at
`10:53:18Z` for Loki and `10:56:00Z` for Tempo. Because the account destinations
are shared, the operator must still visually confirm a
`crosmos-api-production` record and rollout version in each hosted datasource.
