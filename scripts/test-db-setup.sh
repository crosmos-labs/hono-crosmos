...
# One database PER test package. Turbo runs package test tasks in parallel and
...
for DB in "${DBS[@]}"; do
  ...
  # in the test reset emits one per cascaded table, and postgres.js prints each
  # as a structured object — dozens of lines per test, burying the assertions.
  # Setting it on the connection only covers ONE pooled connection.
  psql -h "$HOST" -p "$PORT" -U "$USER" -d postgres -q \
