...
CREATE TABLE "daily_usage" (
	...
	"sources_ingested" integer DEFAULT 0 NOT NULL,
...
CREATE TABLE "organizations" (
	...
	"plan_pending_expires_at" timestamp with time zone,
	"granted_plan" "plan_type",
	"granted_plan_expires_at" timestamp with time zone,
...
CREATE UNIQUE INDEX "uq_connector_space_external_account" ON "connector_connections" USING btree ("space_id","provider","external_account_id") WHERE "connector_connections"."external_account_id" IS NOT NULL AND "connector_connections"."status" IN ('pending', 'active');--> statement-breakpoint
...
CREATE INDEX "organizations_granted_plan_expiry_idx" ON "organizations" USING btree ("granted_plan_expires_at") WHERE granted_plan IS NOT NULL;--> statement-breakpoint
...
CREATE INDEX "memory_spaces_deleted_at_idx" ON "memory_spaces" USING btree ("deleted_at") WHERE deleted_at IS NOT NULL;--> statement-breakpoint
CREATE UNIQUE INDEX "uq_memory_spaces_active_org_id_name" ON "memory_spaces" USING btree ("org_id","name") WHERE deleted_at IS NULL;--> statement-breakpoint
